...
export default async function Page({params}) { const services=await getManagedCatalog("digital-printing",digitalPrinting); const service=services.find((item)=>item.slug===params.slug); if(!service) notFound(); const projects = await getProjectsForService(service.slug); return <CatalogDetail service={service} group="Digital Printing" related={services} projects={projects} />; }
