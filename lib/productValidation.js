...
export const productInputSchema = z.object({
  ...
  if (input.priceTiers.some((item) => item.minQuantity < input.minimumOrderQuantity)) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Price tiers must meet the minimum order quantity." });
  }
  if (new Set(input.priceTiers.map((item) => item.minQuantity)).size !== input.priceTiers.length) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Each price tier needs a unique minimum quantity." });
  }
  if (new Set(input.variants.map((item) => item.name)).size !== input.variants.length) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Each product option needs a unique name." });
  }
});
